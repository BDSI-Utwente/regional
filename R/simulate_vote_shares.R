...
#' @param normalize logical, whether to normalize the vote shares so that they sum to 1. Defaults
#'      to `FALSE`, in which case the user is expected to ensure `func` returns a vector of
...
#'
#'      # we can combine this function with simulate_districts to obtain vote counts.
#'      district_info <- simulate_districts(districts)
#'      vote_shares <- simulate_shares(parties, districts)
#'      vote_shares |>
#'        dplyr::left_join(district_info) |>
#'        # assuming the whole population is eligible to vote for simplicity...
#'        dplyr::mutate(votes = population * turnout * vote_share)
#'
simulate_shares <- function(
  parties,
  districts,
